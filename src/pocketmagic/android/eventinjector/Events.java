...
package pocketmagic.android.eventinjector;
...
public class Events
{
	...
	public class InputDevice {
		...
		public void setUp(int posX,int posY,int trackingid,int purssure){
			REL_X = posX;
			REL_Y = posY;
			TID = trackingid;
			PUR = purssure;
		...
		public int SendTouchAbsXCoord(int x) {
			intSendEvent(m_nId, EV_ABS, REL_X, x); //set x coord
			intSendEvent(m_nId, 0, 0,0);
			return 0;
		}
		public int SendTouchAbsYCoord(int y) {
			intSendEvent(m_nId, EV_ABS, REL_Y, y); //set x coord
			intSendEvent(m_nId, 0, 0,0);
			return 0;
		}
		public int SendTouchDownAbs(int x, int y ) {
			intSendEvent(m_nId, EV_ABS, TID,12345);
			intSendEvent(m_nId, EV_ABS, REL_X, x); //set x coord
			intSendEvent(m_nId, EV_ABS, REL_Y, y); //set y coord
			intSendEvent(m_nId, EV_ABS, PUR,105);
			intSendEvent(m_nId, 0, 0,0);
			/*
		...
		public int SendTouchRelease() {
			intSendEvent(m_nId, EV_ABS, PUR,65535);
			intSendEvent(m_nId, 0, 0,0);
			intSendEvent(m_nId, EV_ABS, TID,MAX);
			intSendEvent(m_nId, 0, 0,0);
			return 1;
		}
